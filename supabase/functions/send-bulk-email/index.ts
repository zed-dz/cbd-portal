...
serve(async (req) => {
  ...
  async function sendWorker() {
    while (cursor < recipients.length) {
      ...
      try {
        const bodyText = renderText(r.name || '', subject, text);
        const bodyHtml = renderHtml(r.name || '', subject, text);
        const d = await deliverOneWay({
          to: r.email, subject, text: bodyText, html: bodyHtml, replyTo: fromEmail,
          gmailToken: accessToken || undefined,
          gmailMime: accessToken ? () => buildMime({ from: fromHeader, to: r.email, subject, text: bodyText, html: bodyHtml }) : undefined,
        });
        results[idx] = d.ok
          ? { email: r.email, ok: true }
          : { email: r.email, ok: false, error: d.detail.slice(0, 200) };
      } catch (e) {
